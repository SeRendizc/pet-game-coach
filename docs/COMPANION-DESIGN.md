...
**当前总体状态一句话**：面试三角色中，**军师**（`coach/strategist.js`）与**老师**（`coach/teacher.js`）各自有完整闭环；**陪练的被动通道**（玩家先开口）现在有状态模型、语气档位、真实事件模板与克制扫描（`coach/companion.js`），**主动通道**（对局中不请自来地说话）有 7 个真实事件、独立于军师的预算，以及自己的出现方式（左下角带头像的一个人）——**2026-09-17 第三轮已接线并在 headless Chrome 里实测到**（详见 §2.2 与 `docs/COMPANION-IMPLEMENTATION.md` §11）。原始执行清单中 **T05「陪练」本身仍然是一个未勾项**（`docs/CHECKLIST.md:70`）：
...
**这条边界已经过时，就地更正**：这里原先写的是「`notify()` 在全仓库没有任何调用处，所以气泡不会弹出」。**那个空档已经补上**：`act()` 每回合结算后都会调用 `companionEvents()`，命中就交给 `notify()` → `queueCompanionCue()`，左下角的气泡会真的出现。第三轮把事件从 2 个扩到 7 个（首次减员、整局结束、连续被克、连着用同一招、局势逆转、长时间僵持、连胜/连败里程碑），并把「什么时候说」的额度与军师彻底分成两份记账（`companionSession()` vs `strategistSession()`）。判定逻辑仍然全部在 `coach/companion.js` 里（纯函数，可脱开 DOM 单测）。
...
| # | 原则 | 理由 |
|---|---|---|
| ~~P1~~ | ~~**不声称内部情绪体验。** 文本里不出现「我难过 / 我开心 / 我生气了」这类第一人称情绪断言~~ **（2026-09-17 第三轮修订，见 §3.5 注记）** | 原判据是「既不可验证，也是油腻的主要来源」。修订后的判据改成：**情绪可以说，但只能指向局面与选择，不能指向玩家的水平**——`voice='sober'` 仍然保留原口径，供「只想被克制地陪」的场景使用 |
...
| # | 禁止模式 | 实现里的正则 | 理由 |
|---|---|---|---|
| 1 | 第一人称情绪断言 | `我(很\|好\|有点\|真的\|也\|现在\|其实)(开心\|难过\|伤心\|生气\|失望\|高兴\|兴奋\|急\|慌\|愣\|心疼…)` | **按声线分开**：`voice='companion'`（默认）放行，`voice='sober'` 仍然拦。放开的是「谁在说话」，不是「能不能训人」 |
| 1b | 说教 | `你应该 / 你必须 / 你最好 / 下次别 / 要记住 / 不该…` | 第三轮新增：说的是「以后你要怎样」就拦 |
| 1c | 战术越界 | `建议你换 / 不如 / 最好… / 换上 / 改用 / 别用 / 先出…` | 第三轮新增：战术指令是军师的活，陪练只评论局面 |
...
2. **一条明确冲突：情绪表达的边界相反。** 小田把「有脾气、会吐槽你瞎指挥」当卖点；小芽的克制扫描把第一人称情绪断言与空泛鼓励**一律拦下**。
   **2026-09-17 第三轮修订（就地更正，不删除原判断）**：产品负责人明确要求陪练「有自己的性格、能跟玩家共情、吐槽」，于是这条边界**按声线拆开**：允许第一人称情绪与对**局面和选择**的轻度吐槽，但**评价玩家水平**仍然是硬线（`skill-insult`），空泛安慰、说教、战术越界也一条不放。原判断里「不放开」的部分仍然成立——小芽防的是 R1 油腻，而油腻的来源不是「有情绪」，是「情绪指向玩家」和「空泛」。**这仍然是「产品定位决定 AI 人格边界」的实例，只是边界换了一个更准的位置。**
