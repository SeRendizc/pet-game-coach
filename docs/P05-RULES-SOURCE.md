...
本轮完成时间：2026-09-17。产物：`engine.js`（新增 `RULES`）、**新增 `rules.js`**、`index.html`、`app.js`、**新增 `rules.test.js`**、`server.js`（模块白名单）。
...
| 位置 | 当时的写法 | 风险 |
|---|---|---|
| `index.html` 规则弹窗 | 整段手写散文：`克制 ×1.5`、`减伤 65%`、`攻击×0.6 − 防御×0.4`、`45 HP`、`回 4 能量`、`80 回合`、`经验 +24 / +12 / +16`、`等级×30`、`生命 +12 / 攻击 +4 / 速度 +3` | 改引擎数值时没人会记得改这里 |
| `app.js` | `b.stacks*15`、`${p.energy}/6`、`'○'.repeat(6-p.energy)`、`Math.min(game.turn,80)`、`首次 10 回合内…`、`v.level*30`、`生命+5 攻防+1` | 同上 |
| `engine.js` | 结算函数里直接写 `1.5`、`.75`、`.6`、`.4`、`.65`、`45`、`6`、`8`、`<=1`、`80` | 数值与同文件的 `desc` 文案也是两份 |
| `coach/runtime.js`（工具层） | `energyLimit:6` | 工具层自己抄了一份能量上限 |

`engine.js` 的 `SKILLS[*].desc` 也是手写字符串，与同对象的 `power/cost/slow/burnBonus/recoil/drain/heal` 字段各写一份。

## 二、改法：一个数字只写一次，界面文案从它派生

### 1. `engine.js` 新增 `RULES`，结算函数改为引用它
...
`withDesc` 让 `desc` 在导出时仍是字符串（`app.js`、`build-knowledge.js` 的用法不变），生成出来的文案与改前逐字相同。开局日志里的 `火克草，草克水，水克火` 也改为由 `typeChartLine()` 从 `TYPE_ADVANTAGES` 生成，与规则弹窗共用同一句。

### 2. 新增 `rules.js`：**不写任何规则数字**的界面文案生成器

`rules.js` 生成 11 节规则页正文（`rulesSections()`），并把机器可读的数值一起导出（`ruleFacts()`）。它的关键做法是：**凡是写在 `progression.js` 里的数值，用引擎自己的函数现场量出来，而不是再抄一份。**

| 界面要显示的 | 来源 |
|---|---|
| 属性倍率、防御减伤、公式系数、状态跳数、能量、道具、携带物、环境、回合上限、等级成长、技能数值 | 直接读 `engine.js` 的 `RULES` / `SKILLS` / `ITEMS` / `HELD_ITEMS` / `ENVIRONMENTS` / `SPECIES` |
...
| 培养格数、单项上限 | 直接读 `progression.js` 的 `trainingCapacity` / `MAX_STAT_TRAINING` |

因此 `rules.js` 里没有 `30`、`24`、`10`、`12` 这些数字，只有索引与判断。
...
`rules.test.js`（16 项，已接入 `npm test`）。核心思路：把引擎**跑起来**，量到真实数字，再和界面上写的数字比对。
...
另外 `offline.test.js` 断言 `rules.js` 与 `engine.js` 的静态 import 图里**没有任何 `coach/` 模块**，所以规则页在未配置密钥、断网时照常可读。
...
统一数据源没有做到 100%。以下是**知道但本轮没改**的地方，原因是它们都在本轮被限定为不可修改的文件里（`coach/*.js`），其中 `coach/companion.js` 同期还在被另一个并行任务重写：
...
其中 `coach/runtime.js` 一项已经用测试兜住；其余三项建议下一轮把 `coach/*.js` 也接进 `rules.js`（改成从 `RULES` / `TRAINING` 取数）。
...
grep -c 'RULES\.' engine.js               # 结算引用统一数据源的处数
```
