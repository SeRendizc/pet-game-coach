...
- **画面**：右侧培养面板：伙伴等级与经验进度、培养格与**三项加点**（耐久/力量/敏捷）、6 选 4 配招与携带物、免费重置。**（更正：原文写"四项加点"，实际 `progression.js:4` 的 `TRAINING` 只有 3 项，`app.js` 也是遍历它。这句话由生成脚本 `scripts/build-demo.mjs:489` 的 `expect` 硬编码，重跑脚本会把这个错误原样再生成一次。）**
- **实测观察**：点击第 2 只伙伴的「培养」后，右栏培养面板没有变化，仍显示「🦊 烬尾狐Lv.1」。核对源码：营地名册的「培养」分支（app.js 第 38 行）只调用 showCamp()，没有重新渲染 #cultivation，所以焦点变了但面板不重绘。脚本用真实鼠标点击和 DOM click 各验证一次，结果一致——这是产品当前的真实行为，不是没点到。**（更正：这个观察在本文件生成后约 1 分钟被 `d4ecd67` 修掉——现在 `app.js:50` 是 `advanceContext();focus=b.dataset.focus;showCamp();cultivation();`，面板会重绘。上面那段文字是 `scripts/build-demo.mjs:486` 的运行期分支（`before===after` 才输出），所以重跑脚本会输出另一条正确分支。）**
...
- **画面**：AI 模式下对方面板只读（提示「已独立出招（看不到你的选择）」），界面结构与真人分屏一致。**（轻微过时：该面板后来改成毛玻璃遮挡，`app.js:311-314`、提交 `0781da2`；上面这句不再完整描述现状。）**
