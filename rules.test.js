...
import {RULES,RULES_VERSION,SKILLS,ITEMS,HELD_ITEMS,ENVIRONMENTS,TYPES,TYPE_ADVANTAGES,SPECIES,
  DIFFICULTIES,createGame,damage,multiplier,legalActions,resolveTurn,typeChartLine,percent} from './engine.js';
import {TRAINING,MAX_STAT_TRAINING,trainingCapacity,newProfile,settle} from './progression.js';
import {ruleFacts,rulesSections,skillLine,piercingSkills} from './rules.js';
...
test('a rules-version mismatch blocks advice instead of showing stale numbers',async()=>{
  const {searchKnowledge,buildKnowledgePacket}=await import('./coach/retrieval.js');
...
test('the difficulty list on the page comes from DIFFICULTIES',()=>{
  ...
  assert.ok(!/option value="easy"/.test(readFileSync(new URL('./index.html',import.meta.url),'utf8')),'index.html 仍在手写难度选项');
});
