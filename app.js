...
$('match-mode').onchange=()=>{const pvp=$('match-mode').value==='pvp';$('opponent-row').hidden=!pvp;$('start').textContent=pvp?'开始对战':'开始训练';$('mode-badge').textContent=(pvp?'对局 · PVP · v0.11':'训练 · PVE · v0.11');$('mode-note').textContent=pvp?'对局中教练照常协助，对手是电脑或真人都一样。同机时对方也可以交接设备来用同一个教练，因此不构成不公平；教练始终不读取对方待执行的动作。':'训练中教练会主动提示并可以随时提问。';};
