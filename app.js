...
let rosterType='all',loadoutDraft=null;
...
function renderLoadout(){
 ...
 const cards=p.learnset.map(id=>{const s=SKILLS[id],on=draft.includes(id);
  return `<button type="button" class="skill-card${on?' on':''}" data-skill="${id}" aria-pressed="${on}" ${!on&&full?'disabled':''}><span class="skill-head"><strong>${s.name}</strong><em>${s.power?`威力 ${s.power}`:'变化'} · ${s.cost} 豆${s.priority?` · 先制 +${s.priority}`:''}</em></span><small>${s.desc}</small></button>`;}).join('');
 box.innerHTML=`<details class="loadout" open><summary>配招 · 6 选 4</summary><p class="muted">选 4 个技能带入下一局，防御也占一个技能槽。改动只影响下一局。</p><div class="skill-grid">${cards}</div><p class="skill-count">已选 <strong>${draft.length}</strong> / 4${draft.length<4?` · 还差 ${4-draft.length} 个`:''}</p><label class="held-row">携带物 <select id="held-item">${Object.entries(HELD_ITEMS).map(([k,x])=>`<option value="${k}" ${held===k?'selected':''}>${x.name}</option>`).join('')}</select></label><p class="muted" id="held-desc">${HELD_ITEMS[held].desc}</p><div class="loadout-actions"><button id="save-loadout" ${draft.length!==4||preview?'disabled':''}>保存配招</button>${loadoutDraft?'<button id="cancel-loadout">放弃修改</button>':''}</div><p id="loadout-error" role="status"></p></details>`;
 ...
 $('save-loadout').onclick=()=>{try{profile=configurePet(profile,focus,loadoutDraft||effectiveLoadout(),$('held-item').value);advanceContext();loadoutDraft=null;save();camp();$('save-message').textContent='配招已保存，下次训练生效';}catch(error){$('loadout-error').textContent=error.message;}};
 const cancel=$('cancel-loadout');if(cancel)cancel.onclick=()=>{loadoutDraft=null;renderLoadout();};
}
