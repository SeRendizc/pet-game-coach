...
import {createGame,legalActions,rankEnemyActions,chooseEnemy} from './engine.js';
import {strategist} from './coach/strategist.js';
import {DIFFICULTY_BRIEFING,briefingFor,legalEnemyChoices,buildOpponentMessages,parseOpponentChoice,decideOpponentAction,enemyAdvice,OPPONENT_TIMEOUT_MS} from './coach/opponent.js';
...
import {legalEnemyActions,enemyFallbackAction,resolveEnemyChoice} from './coach/client.js';
...
test('整局跑通：agent（假模型）与引擎兜底交替出现，每一步都是合法行动',async()=>{
 ...
 while(!g.result&&g.turn<40){
  ...
  const {resolveTurn}=await import('./engine.js');
