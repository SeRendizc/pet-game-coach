...
p('两种对手，同一套规则','h')
p('真人同机：我方选招 → 交接界面提示把设备交给对手 → 对手从自己的合法行动里选一招 → 双方行动一起交给共享结算器结算。若对手的宠倒下，会自动再弹一次交接让对手选补位，由引擎的 replaceQueue 按顺序处理，而不是让 AI 替对手决定。电脑对手：单人也能跑通整条流程，出招由引擎的 chooseEnemy() 按难度决定（随机 / 优先伤害治疗 / 枚举比较），一次模型都不调用。')
p('教练在对局中的介入规则','h')
p('本地对战一律允许教练，对手是电脑或真人都一样。教练是玩家的助手，同机时对方也可以交接设备使用同一个教练，因此不构成不公平。真正该守的底线是「不读取对方待执行的动作」，项目一直遵守且有测试，它不需要靠整局闭麦来实现。')
p('这条规则经过两次修正：初版把所有对局一律闭麦，等于把军师在题目指定的 PVP 场景里禁用；第二版改成「电脑开、真人关」，但电脑对手存在的意义就是模拟真人，两者行为必须一致，逻辑上不自洽。最终统一为本地对战一律开，只保留线上竞技的闭麦开关作为产品钩子。','small')
p('实现边界','h')
table([['项','状态'],['引擎','createGame 接受 mode 与 enemyTeam/enemyPets；buildVersusOpponent 生成可复现的自由阵容；resolveTurn 支持 manualReplace 与 replaceSide；PVE 路径行为不变'],['教练','coach/policy.js 只把 pvp-live 列为闭麦模式；观察层与知识包接受 pve 与 pvp-local'],['已验证','浏览器实测：对局隐藏关卡、对手为随机三只且等级适配；交接→对手合法行动→双方结算推进；电脑对手连打 4 回合无交接层；整局跑完自动复盘；三种模式教练均可用'],['未做','两名真人的多局验收、联网 PVP、权威对局服务。当前是同机轮流，服务端仍接受客户端快照']],[210,330])
p('已知缺陷与修复','h')
p('开发中我曾让电脑对手也走 manualReplace，结果没人替它选补位，对局永久卡在补位阶段。该问题在测试中暴露并已修复：manualReplace 只在真人对手时启用。')
