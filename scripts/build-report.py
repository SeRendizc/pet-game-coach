...
p('如何体验','sub')
p('项目内执行 npm start，打开 http://127.0.0.1:8765/。选择陪伴风格、三只伙伴和难度，开始训练；想演示对局就在「模式」里选本地对战，对手可选真人同机或 AI 代打。DeepSeek 可在 connect.html 加密录入；不配置时保留规则建议，并明确显示本地来源。')
...
p('两种入口，同一套规则','h')
p('营地新增「模式」选择：训练（对电脑）与本地对战（对局）。选本地对战后出现「对手」选择：真人同机（交接设备轮流选招），或 AI 代打（单人也能跑通整条流程，界面明确标注是替身）。')
p('真人同机的回合流程','h')
p('我方选招 → 交接界面（提示把设备交给对手）→ 对手从自己的合法行动里选一招 → 双方行动一起交给共享结算器结算。若对手的宠倒下，会自动再弹一次交接让对手选补位。引擎用 replaceQueue 按顺序处理双方补位，而不是让 AI 替对手决定。')
p('教练规则绑定「对局模式」，不绑定「对手是谁」','h')
p('对局进行中，所有提示界面隐藏，工具层直接拒绝战术查询——因为对面坐着人，任何一方看到提示都不公平。对局结束后恢复正常整局复盘。把规则绑在模式而不是对手上，是为了让行为可预测：不会因为换了个对手，教练就换了性格。AI 代打时同样静默，它是演示替身，不是 PVP。')
p('实现边界','h')
table([['项','状态'],['引擎','createGame 接受 mode；resolveTurn 支持 manualReplace 与 replaceSide；PVE 路径行为不变'],['教练','coach/policy.js 的 isLiveMatch 同时覆盖 pvp-live 与 pvp-local，只在比赛进行中生效'],['已验证','浏览器实测：交接→对手 11 个合法行动→双方结算推进；AI 代打连打 4 回合无交接层且教练静默；整局跑完自动复盘'],['未做','两名真人的多局验收、联网 PVP、权威对局服务。当前是同机轮流，服务端仍接受客户端快照']],[220,320])
p('已知缺陷与修复','h')
p('开发中我曾让 AI 代打也走 manualReplace，结果没人替 AI 选补位，对局永久卡在补位阶段。该问题在测试中暴露并已修复：manualReplace 只在真人对手时启用。')
